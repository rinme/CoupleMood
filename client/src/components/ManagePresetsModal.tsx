...
export const ManagePresetsModal: React.FC<ManagePresetsModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
      <div className="bg-[#FAF7F2] rounded-3xl max-w-lg w-full p-6 sm:p-7 shadow-xl border border-[#E8E2D9] max-h-[90vh] flex flex-col text-[#2D2825]">
        {/* Header */}
        <div className="flex items-start justify-between pb-4 border-b border-[#E8E2D9]">
          <div>
            <h2 className="text-xl font-bold text-[#2D2825]">{t.manageModal.title}</h2>
            <p className="text-xs text-[#8C827A] mt-0.5">{t.manageModal.subtitle}</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label={t.common.close}
            className="p-1.5 rounded-full text-[#8C827A] hover:text-[#2D2825] hover:bg-white active:scale-95 transition-all"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Global Error Banner */}
        {error && (
          <div className="mt-3 p-2.5 bg-rose-50 border border-rose-200 text-rose-700 text-xs rounded-xl flex items-center justify-between">
            <span>{error}</span>
            <button
              type="button"
        ...
        {!isAdding && editingIndex === null && (
          <div className="flex items-center justify-between pt-4 pb-2">
            <button
              ...
              className="flex items-center space-x-1.5 bg-[#2D2825] hover:bg-black text-[#FAF7F2] px-3.5 py-2 rounded-xl text-xs font-semibold shadow-xs active:scale-95 transition-all disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={isSaving}
              className="flex items-center space-x-1 text-xs text-[#8C827A] hover:text-[#2D2825] px-2.5 py-1.5 rounded-xl hover:bg-white active:scale-95 transition-all disabled:opacity-50"
            >
        ...
        {(isAdding || editingIndex !== null) && (
          <form
            ...
            className="my-4 p-4 bg-white/90 rounded-2xl border border-[#E8E2D9] shadow-xs space-y-3.5"
          >
            ...
            <div className="grid grid-cols-4 gap-3">
              ...
              <div className="col-span-1">
                ...
                <input
                  ...
                  maxLength={10}
                  className="w-full text-center text-xl bg-[#FAF7F2] border border-[#E8E2D9] rounded-xl px-2 py-2 focus:outline-none focus:ring-2 focus:ring-rose-400"
                  required
                />
              </div>
              ...
              <div className="col-span-3">
                ...
                <input
                  ...
                  className="w-full text-sm bg-[#FAF7F2] border border-[#E8E2D9] rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-rose-400"
                  required
                />
              </div>
            </div>
                  ...
                  return (
                    <button
                      ...
                      className={`w-7 h-7 rounded-full flex items-center justify-center transition-all ${
                        themeStyle.accent
                      } ${isSelected ? 'ring-2 ring-offset-2 ring-[#2D2825] scale-110' : 'opacity-80 hover:opacity-100'}`}
                    >
            ...
            {/* Form Actions */}
            <div className="flex items-center justify-end space-x-2 pt-2 border-t border-[#E8E2D9]">
              <button
                type="button"
                onClick={handleCancelForm}
                disabled={isSaving}
                className="px-3 py-1.5 text-xs text-[#8C827A] hover:text-[#2D2825] rounded-xl hover:bg-[#FAF7F2] transition-colors"
              >
                {t.common.cancel}
              </button>
              <button
          ...
          {presets.map((preset, index) => {
            ...
            return (
              <div
                ...
                className="flex items-center justify-between p-3 rounded-2xl bg-white/70 border border-[#E8E2D9] hover:bg-white hover:border-[#D6CEC4] transition-all"
              >
                ...
                <div className="flex items-center space-x-1 shrink-0">
                  <button
                    ...
                    title={t.common.edit}
                    className="p-2 rounded-xl text-[#8C827A] hover:text-[#2D2825] hover:bg-[#FAF7F2] active:scale-95 transition-all"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-xl text-[#8C827A] hover:text-rose-600 hover:bg-rose-50 active:scale-95 transition-all disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-[#8C827A]"
                  >
        ...
        {/* Footer */}
        <div className="pt-4 mt-2 border-t border-[#E8E2D9] flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-5 py-2 text-xs font-semibold bg-[#FAF7F2] hover:bg-white border border-[#E8E2D9] rounded-xl text-[#2D2825] active:scale-95 transition-all"
          >
            {t.common.close}
          </button>
        </div>
